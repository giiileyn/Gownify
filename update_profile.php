...
if(isset($_POST['update_profile'])){
   ...
   mysqli_query($con, "UPDATE `users` SET fname = '$update_name', email = '$update_email' WHERE users_id = '$users_id'") or die('query failed');
   ...
   if(!empty($update_pass) || !empty($new_pass) || !empty($confirm_pass)){
      if($update_pass != $old_pass){
         ...
         mysqli_query($con, "UPDATE `users` SET password = '$confirm_pass' WHERE users_id = '$users_id'") or die('query failed');
   ...
   if(!empty($update_image)){
      if($update_image_size > 2000000){
         ...
         $image_update_query = mysqli_query($con, "UPDATE `users` SET image = '$update_image' WHERE users_id = '$users_id'") or die('query failed');
...
<div class="update-profile">
   ...
   <?php
      $select = mysqli_query($con, "SELECT * FROM `users` WHERE users_id = '$users_id'") or die('query failed');
   ...
   <form action="" method="post" enctype="multipart/form-data">
      ...
      <a href="userpanel.php" class="delete-btn">Exit</a>
   </form>
